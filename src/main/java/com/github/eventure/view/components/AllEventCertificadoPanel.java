...
public class AllEventCertificadoPanel extends JPanel{
    public AllEventCertificadoPanel(MainPage mainPage) {
        setLayout(null);
        ...
        JLabel label = new JLabel("Escolha um Evento Para Emitir o certicado", SwingConstants.CENTER);
        ...
        for (int i = 0; i < user.getParticipaoConfirmada().size(); i++) {
            int idEvent = user.getParticipaoConfirmada().get(i);
            var event = eventController.findEventById(idEvent);

            EventPanelForCertificado panel = new EventPanelForCertificado(
                event.getTitle(),
        ...
        int rows = (int) Math.ceil(user.getParticipaoConfirmada().size() / (double) cardsPerRow);
